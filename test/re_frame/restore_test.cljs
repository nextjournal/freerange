...
(use-fixtures :each {:before (partial subs/clear-all-handlers! core/default-frame)})
...
(deftest make-restore-fn-test
  (testing "no existing subs, then making one subscription"
    (register-test-subs)
    (let [original-subs @(:subs-cache core/default-frame)
          restore-fn    (make-restore-fn)]
      (is (zero? (count original-subs)))
      @(subscribe [:test-sub])
      (is (one? (count @(:subs-cache core/default-frame))))
      (is (contains? @(:subs-cache core/default-frame) [[:test-sub] []]))
      (restore-fn)
      (is (zero? (count @(:subs-cache core/default-frame)))))))
...
(deftest make-restore-fn-test2
  (testing "existing subs, making more subscriptions"
    ...
    (let [original-subs @(:subs-cache core/default-frame)
      ...
      @(subscribe [:test-sub2])
      (is (contains? @(:subs-cache core/default-frame) [[:test-sub2] []]))
      (is (two? (count @(:subs-cache core/default-frame))))
      (restore-fn)
      (is (not (contains? @(:subs-cache core/default-frame) [[:test-sub2] []])))
      (is (one? (count @(:subs-cache core/default-frame)))))))
...
(deftest nested-restores
  (testing "running nested restores"
    ...
    (let [restore-fn-1 (make-restore-fn)
          ...
          _            (is (one? (count @(:subs-cache core/default-frame))))
